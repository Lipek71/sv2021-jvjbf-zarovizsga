...
public class TeamController {
    ...
    public TeamDTO createPartner(@Valid @RequestBody CreateTeamCommand command) {
        return teamService.createTeam(command);
    }

    @PostMapping("/{id}/players")
    @Operation(summary = "Add a new player by id")
    public TeamDTO addNewPlayerToTeam(@PathVariable("id") long id, CreatePlayerCommand command){
        return teamService.addNewPlayerToTeam(id, command);
    }

    @PutMapping("/{id}/players")
    @Operation(summary = "Add an existing player by id")
    public TeamDTO addPlayerToTeam(@PathVariable("id") long id, UpdateWithExistingPlayerCommand command){
